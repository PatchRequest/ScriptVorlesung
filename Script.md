...
OSINT wird von verschiedenen Akteuren genutzt:

- **Nationalstaaten**: Geheimdienste wie NSA, BND oder MI6 nutzen OSINT, um Bedrohungslagen zu bewerten und geeignete Maßnahmen zu ergreifen. OSINT dient ihnen dabei, politische Entwicklungen zu analysieren, Bedrohungen zu erkennen und Sicherheitsstrategien zu entwickeln.
- **Strafverfolgungsbehörden**: Behörden wie FBI oder BKA nutzen OSINT, um Straftäter zu ermitteln oder den Kontext von Straftaten zu verstehen. Mithilfe von OSINT können Ermittler gezielt nach Informationen suchen, die in sozialen Medien oder in anderen öffentlichen Quellen veröffentlicht wurden.
- **Kriminelle**: OSINT hilft Kriminellen dabei, Angriffsziele zu identifizieren und gezielte Informationen zu sammeln. Sie nutzen Schwachstellen, die durch öffentlich zugängliche Daten aufgedeckt werden, um Phishing-Angriffe oder Social Engineering zu betreiben.
- **Privatpersonen**: Menschen betreiben OSINT, wenn sie Social-Media-Profile anderer durchstöbern und dabei Informationen analysieren. Auch im Alltag nutzen wir OSINT – zum Beispiel, wenn wir Hintergrundinformationen zu einer Person suchen, die wir treffen wollen.

### Quellen für OSINT

Datenpunkte können aus vielen Quellen stammen. Zu den wichtigsten OSINT-Quellen gehören:

- **Suchmaschinen**
  - **Google Dorking**: Die Verwendung von Suchfiltern wie `inurl:"keyword"` oder `filetype:pdf`, um gezielte Informationen zu finden. Dorking ermöglicht es, Suchmaschinen gezielt zu nutzen, um bestimmte Inhalte zu finden, die nicht auf den ersten Blick sichtbar sind.
  - **Spezialisierte Suchmaschinen**: Tools wie Shodan oder Censys, um gezielt nach Computern, offenen Ports, Fingerprints, Schwachstellen etc. zu suchen. Diese Suchmaschinen helfen dabei, IoT-Geräte, Server und Netzwerke zu identifizieren und deren Sicherheitsstatus zu bewerten.
  - **Achtung: Honey Pots**: Honey Pots sind absichtlich angreifbare Dienste im Internet, die oft in Suchen auftauchen. Nutzer, die mit diesen interagieren, können auf Listen für kommerzielle oder strafrechtliche Zwecke gesetzt werden. Es ist daher wichtig, Interaktionen mit solchen Diensten zu vermeiden.
- **Social Media**: Viele Datenpunkte stammen aus Profilinformationen, Postings und Interaktionen. Social Media Plattformen sind eine wertvolle Informationsquelle, da hier sowohl aktuelle Ereignisse als auch persönliche Interessen und Netzwerke sichtbar werden.
- **Datenbanken**: Zugangsdaten-Leaks, Cloud-Speicher, etc. bieten viele ungewollte Informationen. Diese Datenbanken können Einblicke in kompromittierte Benutzerkonten und andere sensible Informationen bieten.
- **Sonstige Websites**: Foren, Nachrichtenportale, APIs, Videos/TV, KI-basierte Inhalte und vieles mehr. Viele dieser Quellen enthalten wertvolle Hintergrundinformationen oder Kontext zu bestimmten Ereignissen und Personen.
...
Active Directory ist ein komplexes System, das auf einer Kombination verschiedener Protokolle und Systeme basiert. Zu den wichtigsten Komponenten gehören:

- **Kerberos**: Ein Authentifizierungsprotokoll für Client-Server-Umgebungen, das symmetrische Verschlüsselung verwendet und einen hohen Sicherheitsstandard bietet.
- **LDAP**: Ein Protokoll für den Zugriff auf Verzeichnisse, das Basisfunktionen wie Login, Suche, Hinzufügen, Löschen und Ändern von Einträgen ermöglicht.
- **DNS**: Ein hierarchisches System zur Auflösung von Hostnamen in IP-Adressen, das für die Kommunikation und Organisation im Netzwerk unerlässlich ist.
- **SMB**: Ein Protokoll für den Datenaustausch in einem Netzwerk, das den Zugriff auf Dateien und Drucker ermöglicht.

Diese Komponenten werden durch die sogenannte "Microsoft Magic" miteinander verbunden und mit einer grafischen Benutzeroberfläche (GUI) für die einfache Verwaltung versehen.
